...
namespace Emergence.Runtime
{
    public sealed class Fas3SimDriver : MonoBehaviour
    {
        ...
        string _engineSrc, _preludeSrc, _presentationSrc, _checkpointDir = "";
        int _yearTicks = 12;
...
dna:JSON.stringify(E.computeDNA(S))})})()";
        ...
        void Start()
        {
            ...
            try
            {
                ...
                var presPath = EmergenceJintHost.PresentationSourcePath(); // P1 (D-610): optional, sha-guarded in the host
                _presentationSrc = presPath != null ? File.ReadAllText(presPath) : null;
            }
        ...
        void Work()
        {
            try
            {
                var host = new EmergenceJintHost(_engineSrc, _preludeSrc, null, _presentationSrc); // P1 (D-610)
                var eng = host.Engine;
