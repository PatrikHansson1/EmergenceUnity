...
namespace Emergence.Editor
{
    ...
    public static class Fas7SaveLoadPlayerProbe
    {
        ...
        static void Run()
        {
            ...
            // D-123's lesson in PLAYER form (the soak probe's finding): an unfocused player never
            // ticks Update — bake runInBackground into the build; kill any stale hung instance.
            PlayerSettings.runInBackground = true;
            try { Process.Start(new ProcessStartInfo("cmd.exe", "/c taskkill /IM EmergenceSaveLoad.exe /F") { CreateNoWindow = true, UseShellExecute = false }); System.Threading.Thread.Sleep(1500); } catch { }
