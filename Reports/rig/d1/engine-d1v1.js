...
function createWorld(seed,founders){
  const S={
    ...
    events:[],knowledge:{},customs:{},nextCustomId:1,hypos:{},nextHypoId:1,
...
// D1 varv 1 (D-701): which open need window covers this tech (mirror of needBoost's windows).
...
function talk(S,a,b){
  ...
  if(a.rel[b.id]>45&&b.rel[a.id]>45&&a.age>16&&b.age>16&&a.age<52&&b.age<52
     &&a.childCd===0&&b.childCd===0&&a.hunger>40&&b.hunger>40&&((S._aliveN||S.agents.filter(x=>!x.dead).length)+(S.aggregates&&S.aggregates.length?S.aggregates.reduce((t,g)=>t+g.cohorts[0]+g.cohorts[1]+g.cohorts[2]+g.cohorts[3],0):0))<carryingCapacity(S) // E1 MALTHUS: the land sets the ceiling (cached alive count)
...
// B2.2a (D-497): AGGREGATSWITCHEN — vilande skyddsnät. T_AGG/T_DE via S._aggT (test kan sänka).
function _fnvh(s){let h=2166136261>>>0;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619)>>>0;}return h;}
function _bearerScore(a){return a.knows.size*2+(a.epithets?a.epithets.length:0)+a.age/10;}
function _catchments(S){
  const map=new Map(); for(const v of S.villages)map.set(v,[]);
  for(const a of S.agents){ if(a.dead)continue; let bd=1e9,bv=null;
    for(const v of S.villages){const d=Math.hypot((a.home?a.home.x:a.x)-v.x,(a.home?a.home.y:a.y)-v.y);if(d<bd){bd=d;bv=v;}}
    if(bv)map.get(bv).push(a); }
  return map;
}
...
function aggregateTick(S){
  ...
  for(let i=S.aggregates.length-1;i>=0;i--){
    const g=S.aggregates[i];
    // v3 (D-498): EKONOMIKOPPLINGEN — vuxenkohorten räknas som fältarbete (fyller fältgapet mot 24-taket)
    const _fgap=Math.max(0,24-Math.min(S.fields.length,24));
    const _cpop=g.cohorts[0]+g.cohorts[1]+g.cohorts[2]+g.cohorts[3];
    // v6 (D-499): BÄRYTAN är befolkningsdriven (seedsSprout-ekologin) — kohortmassan 'äter-och-sår'.
    // Kalibrering ur jämviktsanalys: ref-värld ~275 sites vid pop 391, agg-värld ~204 vid ~15 aktiva
    // ⇒ befolkningsbidrag ≈ (275-204)/(391-15) ≈ 0,19 sites/själ. Term: cpop*0,19*0,28*KLIFT.
    const _bonus=_cpop*0.19*0.28;
    const K=Math.max(1,carryingCapacity(S)+_bonus);
    let c=g.cohorts, pop=c[0]+c[1]+c[2]+c[3];
    ...
    if(pop+bearersAlive<T.de){
      ...
      const bounds=[[1,13],[14,39],[40,64],[65,80]];
      for(let ci=0;ci<4;ci++){ const n=Math.round(c[ci]);
        for(let k=0;k<n;k++){ const h=_fnvh(String(S.seed)+'/'+g.village+'/'+yr+'/'+ci+'/'+k);
          const age=bounds[ci][0]+h%(bounds[ci][1]-bounds[ci][0]+1);
          const a=makeAgent(S,(v?v.x:50)+((h>>>8)%7)-3,(v?v.y:35)+((h>>>16)%7)-3,null,false);
          a.age=age;
          for(const t in g.traitsM){const mu=g.traitsM[t].mean,sd=g.traitsM[t].sd;
            a.traits[t]=clamp(mu+((h>>>24)/255-0.5)*2*sd,0.05,0.95);}
  ...
  for(const [v,mem] of catch_){
    ...
    if(alive.length>=T.agg || (_totAlive>_POPCAP && alive.length===_maxC && alive.length>=T.de+15)){ // H0(a) RIGG D-617: fallback-fold aldrig under de-fold-tröskeln (pop+bärare ≥ T.de)
      ...
      const c=[0,0,0,0]; const knows=new Set(); const tM={curiosity:{s:0,q:0},social:{s:0,q:0},diligence:{s:0,q:0}};
      for(const a of fold){ const ci=a.age<14?0:(a.age<40?1:(a.age<65?2:3)); c[ci]++;
        for(const k of a.knows)knows.add(k);
        for(const t in tM){const x=a.traits[t]||0.5;tM[t].s+=x;tM[t].q+=x*x;}
        a.dead=true; a._folded=true; if(a.home)a.home.free=true; // viks in — INTE killAgent (inga dödsevent för massan)
      }
...
// B4 v1 (D-507/D-508): ENDEMISK SJUKDOM — täthetens pris (McNeill-benet).
// Feberns år: tryck q=0,02·max(0,N−50)/50 per person-år över upptagningsområdets massa
// (levande + kohortmassa — B2.2a-kontraktet), barn ×2/äldre ×3, medicine-bärare ×0,5,
// aqueduct ×0,7. Deterministisk; ingen ny rand-ström; glesa världar hör aldrig feberns år.
// Rigg-dom D-508: M1 2,19‰/0,00‰ · M2 1v1 utdöda, −6,5% pop · M4 Δ0,3pe · M5 80%/0 läckage.
function sicknessTick(S){
  const catch_=_catchments(S);
  for(const pair of catch_){
    const v=pair[0], mem=pair[1];
    const alive=mem.filter(a=>!a.dead);
    let cmass=0, g=null;
    if(S.aggregates&&S.aggregates.length){ g=S.aggregates.find(g2=>g2.village===v.name)||null;
      if(g)cmass=g.cohorts[0]+g.cohorts[1]+g.cohorts[2]+g.cohorts[3]; }
    const N=alive.length+cmass;
    const q0=0.02*Math.max(0,N-50)/50;
    ...
    for(const a of alive){
      if(a.dead)continue;
      const w=a.age<14?2:(a.age>=65?3:1);
      if(S.rand()<q0*aq*w*hm){
        died++;
