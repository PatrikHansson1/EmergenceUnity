...
const out=path.join(__dirname,'..','d1','d1k2-results.txt');
...
for(const [seed,f] of SEEDS){
  ...
  const dist={};for(const h of hy)dist[h.trials]=(dist[h.trials]||0)+1;const distS={};for(const h of hy){const b=distS[h.status]||(distS[h.status]={});b[h.trials]=(b[h.trials]||0)+1;}
  ...
  fs.appendFileSync(out,JSON.stringify({tag,endYear:Math.floor(S.tick/YEAR),ended:S.ended,secs:Math.round((Date.now()-t0)/1000),
    ...
    maxTrials:Math.max(0,...hy.map(h=>h.trials)),mutations:hy.reduce((s,h)=>s+h.mutations,0),trialsDist:dist,trialsDistByStatus:distS,immortalAfterSweep:immortal,
