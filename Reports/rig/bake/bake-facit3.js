'use strict';
// bake-facit3.js — re-record the deep-time facits (4242 + 2323, t=172800 = 1200y) on the LIVE engine
// (v24 e2285e55 = v23 + mktext text-rev; D-758). Windows-native via RUN_NODEBAKE. Intentional divergence
// from the v21 facits: D1 (v22/v23, ways from failed hypotheses, >= year 378) + text fixes F1/F3/F6/F7.
// Writes NEW files facit-seed-<s>-t172800.v24.txt in deeptime-facit/ — promotion to .canon.txt (with
// .9da69373.bak backup of the v21 facits) happens in a separate, eyeballed step after a byte-identical re-run. Loads the V8-hypot prelude first (parity with all probe contexts).
...
const log=path.join(__dirname,'bake-facit3.progress.txt');
fs.appendFileSync(log,'# start '+new Date().toISOString()+' engineSha='+engSha+' node='+process.version+'\n');
if(engSha.slice(0,8)!=='e2285e55'){fs.appendFileSync(log,'# ABORT wrong engine sha\n');process.exit(1);}
for(const seed of [4242,2323]){
  ...
  const f=path.join(outDir,'facit-seed-'+seed+'-t172800.v24.txt');
