// m4.js — M-D1 KONFIRMATORISK M4-MÄTNING (prereg 6e6fbedd, D-715; §5): 20 världar × 3000 år.
// 12 färska M4-frön (FÖRSTA körningen någonsin) + 8 kanonfrön. INGEN xp. INGEN tolkning här —
// skriptet bokför rå avläsning per värld; domen sker vid 20/20 per §6. Resumbart: frön med
// komplett rad i m4-results.txt hoppas över (ofullbordad värld körs om identiskt, §5/G11).
...
const out=path.join(__dirname,'..','d1','m4-results.txt');
...
for(const job of JOBS){
  ...
  const prog=path.join(__dirname,'..','d1','m4-progress-'+job.tag+'.csv');
  fs.writeFileSync(prog,'year,pop,secs\n');
  const S=job.f?E.createWorld(job.seed,job.f):E.createWorld(job.seed);S.silent=true;
  let at1200=null,lastProgYear=0;
  while(Math.floor(S.tick/YEAR)<3000&&!S.ended){
    ...
    if(y>=lastProgYear+100){lastProgYear=y;fs.appendFileSync(prog,y+','+S.agents.filter(a=>!a.dead).length+','+Math.round((Date.now()-t0)/1000)+'\n');}
  }
...
if(done2.size>=20)fs.appendFileSync(out,'# DONE 20/20 '+new Date().toISOString()+'\n');
else fs.appendFileSync(out,'# PARTIAL '+done2.size+'/20 '+new Date().toISOString()+'\n');
