...
class Queue {
  ...
  push(tick, cls, id, fn, epoch) {
    const e = { tick, cls, id, seq: this.seq++, fn, epoch };
    this.h.push(e);
...
const PROCESS = [
  { id: 'heat',    need: d => d.thresh >= 1,                 gives: d => ({ hard: +2, plastic: -2 }) },
...
function createWorld(seed, opts = {}) {
  const W = {
    ...
    memo: new Set(),                // ★ metod 3: nedskrivna misslyckanden
...
function attempt(W, key) {
  ...
  if (!ok) { if (W.methods.written) W.memo.add(tk); return null; }
  ...
  if (!aff.length) { if (W.methods.written) W.memo.add(tk); return null; }
...
function keep(W, c) {
  const score = a => {
    let s = 0; for (const k in c.out.d) s += c.out.d[k];
    return s + c.aff.length * 3 + (c.comp ? 4 : 0);
  };
  const sc = score(c);
  // a) höjer bäst-hittills på någon affordans
  for (const a of c.aff) if (!(a in W.best) || sc > W.best[a]) return 'a';
  // b) uppfyller ett processpredikat inget befintligt (material, form) uppfyller
  for (const p of PROCESS) if (p.need(c.out.d) && !W.mats.some(m => p.need(m.d))) return 'b';
...
function schedule(W) {
  ...
  W.q.push(W.now + YEAR, CLASS.CALENDAR, 0, yearTick, 0);
}
...
function yearTick(W) {
  ...
  for (let i = 0; i < searchers; i++) {
    ...
    else if (W.methods.written) W.memo.add(c.tk);
  }
  schedule(W);
}
...
function scheduleMethods(W) {
  W.q.push(30 * YEAR, CLASS.THRESHOLD, 1, w => { w.methods.craft = true; w.log.push([w.year, 'method', 'hantverket — lokal sökning']); }, 0);
  W.q.push(400 * YEAR, CLASS.THRESHOLD, 2, w => { w.methods.written = true; w.log.push([w.year, 'method', '★ den nedskrivna misslyckade prövningen — memoisering']); }, 0);
}

/** INSTRUMENT öppnar DIMENSIONER. Det enda som håller egenskapsrummet öppet. */
function scheduleInstruments(W) {
  [700, 1400, 2100].forEach((y, i) => {
    W.q.push(y * YEAR, CLASS.THRESHOLD, 10 + i, w => {
      w.visibleDims++; w.instruments++;
...
function scheduleDemand(W, n) {
  ...
  W.q.push(W.now + wait * YEAR, CLASS.DEMAND, n, w => {
    const d = DEMANDS[drawI(w, 3, n, 2, DEMANDS.length)];
    if (!w.demand.has(d)) { w.demand.add(d); w.log.push([w.year, 'demand', 'marken kräver: ' + d]); }
...
function run(seed, years, opts = {}) {
  ...
  schedule(W); scheduleMethods(W); scheduleInstruments(W); scheduleDemand(W, 0);
...
module.exports = { run, createWorld, DIM, FORM, PROCESS, AFFORD, tripleKey, Queue, draw, waitYears };
