...
function adopt(W, c, why) {
  ...
  const nh = maxHeat(W);
  if (nh > W.heatCap) { W.heatCap = nh; W.epoch++; W.log.push([W.year, 'heat', 'varmegransen stiger till ' + nh + ' via ' + nm]); }
...
function onInstrument(W) {
  W.visibleDims++; W.instruments++; W.epoch++;
...
function onDemand(W, n) {
  ...
  if (!W.demand.has(d)) { W.demand.add(d); W.epoch++; W.log.push([W.year, 'demand', 'marken kraver: ' + d]); }
...
function onPressure(W, n) {
  ...
  if (on) { W.demand.add(a); W.epoch++; W.log.push([W.year, 'pressure', k + ' kraver: ' + a]); }
