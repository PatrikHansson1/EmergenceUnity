...
const EV = { YEAR: 0, METHOD: 1, INSTRUMENT: 2, DEMAND: 3 };
class Queue {
...
function keep(W, c) {
  ...
  for (const p of PROCESS) {
    if (p.tier !== undefined && p.tier > W.heatCap) continue;      // onaabar niva raknas inte
    if (p.need(c.out.d) && !W.mats.some(m => m !== c.out && p.need(m.d))) return 'b';
  }
...
function adopt(W, c, why) {
  ...
  const nh = maxHeat(W);
  if (nh > W.heatCap) { W.heatCap = nh; W.log.push([W.year, 'heat', 'varmegransen stiger till ' + nh + ' via ' + nm]); }
  let s = 0; for (const k in c.out.d) s += c.out.d[k];
...
function onDemand(W, n) {
  ...
  if (!W.demand.has(d)) { W.demand.add(d); W.log.push([W.year, 'demand', 'marken kraver: ' + d]); }
  scheduleDemand(W, n + 1);
}

const HANDLER = { [EV.YEAR]: yearTick, [EV.METHOD]: onMethod, [EV.INSTRUMENT]: onInstrument, [EV.DEMAND]: onDemand };
